...
type Config struct {
	...
	JobStatus         string   `json:"jobs_status"` //ztt
...
func (r *LogExportRunner) readDatas(dr reader.DataReader, dataSourceTag string, l rate.Limiter, c context.Context) []Data {
	var (
		datas     []Data
		err       error
		bytes     int64
	...
	for !utils.BatchFullOrTimeout(r.RunnerName, &r.stopped, r.batchLen, r.batchSize, r.lastSend,
		...
		d[parserconfig.KeyRaw] = string(marshal)
		//d[parserconfig.KeyRaw] = data
		d[parserconfig.KeyTimestamp] = time.Now().UnixNano() / 1e6
		//ztt
		datas = append(datas, d)
		r.batchLen++
		r.batchSize += bytes + 180
	}
	r.rsMutex.Lock()
	if err != nil {
...
func (r *LogExportRunner) rawReadLines(dataSourceTag string, l rate.Limiter, c context.Context) (lines, froms []string) {
	var line string
	var err error
	for !utils.BatchFullOrTimeout(r.RunnerName, &r.stopped, r.batchLen, r.batchSize, r.lastSend,
		r.MaxBatchLen, r.MaxBatchSize, r.MaxBatchInterval) {
...
func (r *LogExportRunner) readLines(dataSourceTag string, l rate.Limiter, c context.Context) []Data {
	var (
		err        error
		curTimeStr string
	)
	lines, froms := r.rawReadLines(dataSourceTag, l, c)
...
func (r *LogExportRunner) Run() {
	...
	for {
		...
		r.tracker.Reset()
		if r.SendRaw {
			lines, _ := r.rawReadLines(r.meta.GetDataSourceTag(), *l, c)
			r.tracker.Track("finish rawReadLines")
		...
		if dr, ok := r.reader.(reader.DataReader); ok {
			datas = r.readDatas(dr, r.meta.GetDataSourceTag(), *l, c)
			r.tracker.Track("finish readDatas")
		} else {
			datas = r.readLines(r.meta.GetDataSourceTag(), *l, c)
		...
		if len(datas) >= 100 {
			newSlince := datas[1:100]
			fifo.Put(equeue.Element{Key: r.Name(), Value: newSlince})
		} else {
			fifo.Put(equeue.Element{Key: r.Name(), Value: datas})
		}
		...
		go func(url string) {
			...
			for e := fifo.Equeue.Front(); e != nil; e = e.Next() {
				Value := e.Value.(equeue.Element).Value
				jsonData, err = json.Marshal(Value)
				stringData = string(jsonData)
				//utils.HttpCall("POST", url, strings.NewReader(stringData))
				if len(stringData) > 0 {
					//utils.HttpCall("POST", url, strings.NewReader(stringData))
					utils.HttpCallToken("POST", url, strings.NewReader(stringData), token)
				}
			}
		}(url)
		//ztt
