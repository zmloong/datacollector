...
func NewReader(meta *reader.Meta, conf conf.MapConf) (reader.Reader, error) {
	...
	brokerList, _ := conf.GetStringList(KeyKafkaZookeeper)
	...
	kr.Consumer, consumerErr = saramaCluster.NewConsumer(
		kr.brokerList,
		kr.groupID,
		kr.Topics,
		config,
...
func (r *Reader) Name() string {
	return fmt.Sprintf("KafkaReader:[%s],[%s]", strings.Join(r.Topics, ","), r.groupID)
}
...
func (r *Reader) Source() string {
	return fmt.Sprintf("[%s],[%s]", strings.Join(r.Topics, ","), r.groupID)
}
...
func (r *Reader) markOffset() {
	...
	for topic, partOffset := range r.currentOffsets {
		...
		for partition, offset := range partOffset {
			r.Consumer.MarkOffset(&sarama.ConsumerMessage{
				Topic:     topic,
				Partition: partition,
				Offset:    offset,
			}, "")
