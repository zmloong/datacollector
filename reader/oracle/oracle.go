...
var (
	_ reader.DaemonReader = &OracleReader{}
	_ reader.StatsReader  = &OracleReader{}
	_ reader.DataReader   = &OracleReader{}
	_ reader.Reader       = &OracleReader{}
)

var OracleSystemDB = []string{"information_schema", "performance_schema", "oracle", "sys"}

const OracleTimeFormat = "2006-01-02 15:04:05.000000"

func init() {
	reader.RegisterConstructor(ModeOracle, NewOracleReader)
}
...
func NewOracleReader(meta *reader.Meta, conf conf.MapConf) (reader.Reader, error) {
	var (
	...
	readBatch, _ = conf.GetIntOr(KeyOracleReadBatch, 1000)
	offsetKey, _ = conf.GetStringOr(KeyOracleOffsetKey, "INCREMENTAL")
	if logpath == "" {
		dataSource, err = conf.GetPasswordEnvString(KeyOracleDataSource)
	} else {
		dataSource, err = conf.GetPasswordEnvStringOr(KeyOracleDataSource, logpath)
	}
	if err != nil {
		return nil, err
	}

	timestampkey, _ = conf.GetStringOr(KeyOracleTimestampKey, "")
	timestampInt, _ = conf.GetBoolOr(KeyOracleTimestampInt, false)
	if !timestampInt {
		startTime = time.Now()
		startTimeStr, _ = conf.GetStringOr(KeyOracleStartTime, "")
		if startTimeStr != "" {
		...
		timestampDurationStr, _ := conf.GetStringOr(KeyOracleBatchDuration, "1m")
		batchDuration, err = time.ParseDuration(timestampDurationStr)
		if err != nil {
			return nil, err
		}
	} else {
		startTimeInt, _ = conf.GetInt64Or(KeyOracleStartTime, 0)
		batchDurInt, _ = conf.GetIntOr(KeyOracleBatchDuration, 1000)
	}

	rawDatabase, err = conf.GetString(KeyOracleDataBase)
	if err != nil {
		return nil, err
	}
	rawSQLs, _ = conf.GetStringOr(KeyOracleSQL, "")
	_, _ = conf.GetStringOr(KeyOracleCron, "")
	execOnStart, _ = conf.GetBoolOr(KeyOracleExecOnStart, true)
	encoder, _ = conf.GetStringOr(KeyOracleEncoding, "utf8")
	if strings.Contains(encoder, "-") {
		encoder = strings.Replace(strings.ToLower(encoder), "-", "", -1)
	}
	param, _ := conf.GetStringOr(KeyOracleParam, "")
	if param != "" {
		param = strings.TrimSpace(param)
		param = strings.Trim(param, "&")

	}
	historyAll, _ = conf.GetBoolOr(KeyOracleHistoryAll, false)
	table, _ = conf.GetStringOr(KyeOracleTable, "")
	...
	r.keepRawSql, _ = conf.GetStringOr(KeyOracleSQL, "")
	r.parserKeepSql()
	if r.rawDatabase == "" {
		r.rawDatabase = "*"
	}
	...
	if !omitMeta {
		...
		if len(tablePrefix) > 0 {
			if strings.Contains(tablePrefix, "-") {
...
func (r *OracleReader) isStopping() bool {
	return atomic.LoadInt32(&r.status) == StatusStopping
}

func (r *OracleReader) hasStopped() bool {
	return atomic.LoadInt32(&r.status) == StatusStopped
}

func (r *OracleReader) Name() string {
	return "Oracle_Reader:" + r.rawDatabase + "-" + models.Hash(r.rawSQLs)
}

func (r *OracleReader) SetMode(mode string, v interface{}) error {
	return errors.New("Oracle reader does not support read mode")
}

func (r *OracleReader) setStatsError(err string) {
	r.statsLock.Lock()
	r.stats.LastError = err
	r.statsLock.Unlock()
}

func (r *OracleReader) sendError(err error) {
	if err == nil {
		return
	}
	defer func() {
...
func (r *OracleReader) Start() error {
...
func (r *OracleReader) Source() string {
	// 不能把 DataSource 弄出去，包含密码
	return "ORACLE_" + r.database
}

func (r *OracleReader) ReadLine() (string, error) {
	return "", errors.New("method ReadLine is not supported, please use ReadData")
}

func (r *OracleReader) ReadData() (models.Data, int64, error) {
...
func (r *OracleReader) Status() models.StatsInfo {
	r.statsLock.RLock()
...
func (r *OracleReader) updateOffsets(sqls []string) {
	r.muxOffsets.Lock()
...
// Oracle 中 若原始sql语句为空，则根据用户填写的database, table, historyAll获取数据，并且统计数据的条数
...
func (r *OracleReader) exec(connectStr string) (err error) {
	var (
		now = time.Now().Add(-r.magicLagDur)
		// 获取符合条件的数据库
...
func (r *OracleReader) execDB(dbs []string, now time.Time, handlerFunc int) error {
...
func (r *OracleReader) execCountDB(curDB string, now time.Time, recordTablesDone TableRecords) error {
	connectStr := r.getConnectStr(curDB, now)
	db, err := OpenSql(ModeOracle, connectStr)
	if err != nil {
		return err
	}
	defer db.Close()

	log.Infof("Runner[%v] prepare Oracle change database success, current database is: %v", r.meta.RunnerName, curDB)
...
func (r *OracleReader) execReadDB(curDB string, now time.Time, recordTablesDone TableRecords) (err error) {
	connectStr := r.getConnectStr(curDB, now)
	db, err := OpenSql(ModeOracle, connectStr)
	if err != nil {
		return err
	...
	log.Debugf("Runner[%v] %v prepare Oracle change database success", r.meta.RunnerName, curDB)
	r.database = curDB
				...
				if exit {
					...
					if len(r.startDate) > 0 {
						...
						if canIncrease {
							...
							if len(r.rawSQLs) > 0 {
								if strings.Contains(r.tablePrefix, "-") {
...
func (r *OracleReader) isRecordSqls() bool {
	return r.timestampKey == "" && r.offsetKey == "" && r.rawSQLs != ""
}

func (r *OracleReader) getSQL(idx int, rawSQL string) string {
	r.muxOffsets.RLock()
	defer r.muxOffsets.RUnlock()
...
func (r *OracleReader) checkExit(idx int, db *sql.DB) (bool, int64) {
	...
	if len(r.timestampKey) > 0 {
		...
		*/
		...
		if r.timestampKeyInt {
			...
			if r.startTimeStr == "" {
				tsql = fmt.Sprintf("select COUNT(*) as countnum %v WHERE %v >= '%s';", rawSQL, r.timestampKey, r.startTime.Format(OracleTimeFormat))
			} else {
		...
		if r.timestampKeyInt {
			...
			if r.startTimeStr == "" {
				tsql = fmt.Sprintf("select MIN(%s) as %s %v WHERE %v > '%s';", r.timestampKey, r.timestampKey, rawSQL, r.timestampKey, r.startTime.Format(OracleTimeFormat))
			} else {
				tsql = fmt.Sprintf("select MIN(%s) as %s %v WHERE %v > '%s';", r.timestampKey, r.timestampKey, rawSQL, r.timestampKey, r.startTimeStr)
			}
		}
	} else {
		tsql = fmt.Sprintf("%s WHERE %v >= %d and %v < %d order by %v", rawSQL, r.offsetKey, r.offsets[idx], r.offsetKey, r.offsets[idx]+1, r.offsetKey)
	}
...
func (r *OracleReader) getConnectStr(database string, now time.Time) string {
...
func (r *OracleReader) Lag() (rl *models.LagInfo, err error) {
	rl = &models.LagInfo{SizeUnit: "records"}
	return rl, nil
}

func (r *OracleReader) checkCron() bool {
	return r.isLoop || r.cronSchedule
}

// 计算每个table的记录条数
func (r *OracleReader) execTableCount(connectStr string, idx int, curDB, rawSql string) (tableSize int64, err error) {
...
func (r *OracleReader) getAllDatas(rows *sql.Rows /*, scanArgs []interface{}*/, columns []string, nochiced []bool) ([]ReadInfo, bool, [][]byte) {
	datas := make([]ReadInfo, 0)
	scanArgs := make([]interface{}, len(columns))
	rawResult := make([][]byte, len(columns))
	for i := range rawResult {
		scanArgs[i] = &rawResult[i]
	}
	for rows.Next() {
		err := rows.Scan(scanArgs...)
		if err != nil {
		...
		if r.isStopping() || r.hasStopped() {
			log.Warnf("Runner[%v] %v stopped from running", r.meta.RunnerName, r.Name())
			return nil, true, rawResult
		}
		datas = append(datas, ReadInfo{Data: data, Bytes: totalBytes})
	}
	return datas, false, rawResult
}

// 执行每条 sql 语句
func (r *OracleReader) execReadSql(curDB string, idx int, execSQL string, db *sql.DB) (exit bool, readSize int64, err error) {
	exit = true
	...
	_, nochiced := GetInitScans(len(columns), rows, r.schemas, r.meta.RunnerName, r.Name())
	var offsetKeyIndex int
	if r.rawSQLs != "" {
		offsetKeyIndex = GetOffsetIndexWithTimeStamp(r.offsetKey, r.timestampKey, columns)
	}

	alldatas, closed, scanArgsnew := r.getAllDatas(rows /*scanArgs, */, columns, nochiced)
	if closed {
	...
	for _, v := range alldatas {
		...
		if len(r.timestampKey) <= 0 {
			maxOffset = r.updateOffset(idx, offsetKeyIndex, maxOffset, scanArgsnew)
		}
	}
...
func (r *OracleReader) updateOffset(idx, offsetKeyIndex int, maxOffset int64, scanArgs [][]byte) int64 {
	if offsetKeyIndex >= 0 {
		stringKeyOffest := string(scanArgs[offsetKeyIndex])
		tmpOffsetIndex, err := strconv.ParseInt(stringKeyOffest, 10, 64)
		//tmpOffsetIndex, err := ConvertLong(stringKeyOffest)
		if err != nil {
...
func (r *OracleReader) addCount(current int64) {
	r.countLock.Lock()
...
func (r *OracleReader) parserSql() {
	if strings.Contains(r.rawSQLs, "rownum") || strings.Contains(r.rawSQLs, "ROWNUM") {
		r.rawSQLs = "SELECT * FROM (" + r.keepRawSql + ")"
	}
}

func (r *OracleReader) parserKeepSql() {
	if strings.Contains(r.keepRawSql, "rownum") || strings.Contains(r.keepRawSql, "ROWNUM") {
		r.keepRawSql = "SELECT * FROM (" + r.keepRawSql + ")"
	}
}
...
func (r *OracleReader) getTableFromMeta(rawMeta string) string {
