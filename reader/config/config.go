...
var (
	...
	ModeToolTips = KeyValueSlice{
		...
		{ModeDir, "datacollector会在启动时根据文件夹下文件时间顺序依次读取文件，当读到时间最新的文件时会不断读取追加的数据，直到该文件夹下出现新的文件。该模式的经典日志存储方式为整个文件夹下存储业务日志，文件夹下的日志使用统一前缀，后缀为时间戳，根据日志的大小rotate到新的文件。", ""},
		{ModeFile, "datacollector会不断读取文件追加的数据。该模式的经典日志存储方式类似于nginx的日志rotate方式，日志名称为固定的名称，如access.log,rotate时直接move成新的文件如access.log.1，新的数据仍然写入到access.log。", ""},
		...
		{ModeElastic, "Elasticsearch Reader 是datacollector提供的从Elasticsearch读取日志的配置方式。Elasticsearch Reader输出的是json字符串，需要使用json的parser解析。", ""},
		{ModeMongo, "MongoDB reader 是datacollector提供的从MongoDB读取数据的配置方式。MongoDB reader 输出的是json字符串，需要使用json的parser解析。", ""},
		{ModeKafka, "Kafka reader 是datacollector提供的从Kafka读取数据的配置方式。针对0.8及以前版本的Kafka服务", "Kafka"},
		{ModeRedis, "Redis Reader 是datacollector提供的从Redis读取日志的配置方式。Redis Reader 输出的是redis中存储的字符串，具体字符串是什么格式，可以在parser中用对应方式解析。", ""},
		{ModeSocket, `Socket Reader 是datacollector提供的以端口监听的方式接受并读取日志的形式，主要支持tcp\udp\unix套接字 这三大类协议。`, ""},
		{ModeHTTP, `Http Reader 是 datacollector 提供的以 http post 请求的方式接受并读取日志的形式。该 reader 支持 gzip, 但请在请求头中添加Content-Encoding=gzip 或者 Content-Type=application/gzip，默认接收 request body 中所有的数据作为要读取的日志, 限制 request body 小于 100MB，默认将 request body 中的数据使用 \n 分割, 每行作为一条数据`, ""},
...
var (
	...
	OptionDelimiter = Option{
		...
		Default:      "",
		DefaultNoUse: false,
		Description:  "换行符(delimiter)",
		Advance:      true,
		ToolTip:      "读取文件的换行符，默认根据编码方式获取换行符，如果包含特殊字符，需要加上双引号，如:\"\\n\"",
	}
...
var ModeKeyOptions = map[string][]Option{
	...
	ModeElastic: {
		...
		},
		{
			...
			CheckRegex:   "\\d+[dhms]",
			Advance:      true,
			ToolTip:      "datacollector重启后可以继续读取ES数据的Offset记录在es服务端保存的时长，默认1d",
		},
		{
	...
	ModeKafka: {
		{
			...
			Default:      "",
			Required:     true,
			Placeholder:  "datacollector1",
			DefaultNoUse: true,
			Description:  "consumer组名称(kafka_groupid)",
			ToolTip:      "kafka组名，多个datacollector同时消费时写同一个组名可以协同读取数据",
		},
		{
	...
	ModeSnmp: {
		{
			...
			ChooseOnly:   false,
			Default:      "",
			Placeholder:  "datacollector_default_name",
			DefaultNoUse: true,
